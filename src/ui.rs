...
fn render_banner(f: &mut Frame, area: Rect, theme: &Theme, i18n: &crate::i18n::I18n) {
    let banner_text = vec![
        Line::from(vec![
            Span::styled("🔐  ", Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2))),
            Span::styled("AGESMITH", Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2)).add_modifier(Modifier::BOLD)),
            Span::styled("  v", Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2))),
            Span::styled(VERSION, Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2))),
            Span::styled("  │  ", Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2))),
            Span::styled(i18n.t("app_tagline"), Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2)).add_modifier(Modifier::ITALIC)),
        ]),
    ];
...
fn render_settings_modal(f: &mut Frame, app: &App, area: Rect) {
    ...
    let block = Block::default()
        ...
        .title_style(Style::default().fg(Color::Rgb(theme.primary.0, theme.primary.1, theme.primary.2)).add_modifier(Modifier::BOLD))
        .border_style(Style::default().fg(Color::Rgb(theme.success.0, theme.success.1, theme.success.2)))
    ...
    let theme_style = if app.settings_selected == 0 {
        Style::default().fg(Color::Rgb(theme.warning.0, theme.warning.1, theme.warning.2)).add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(Color::Rgb(theme.fg.0, theme.fg.1, theme.fg.2))
    };
    let theme_text = vec![
        Line::from(format!("{}: {} {}", app.i18n.t("settings_theme"), app.config.theme, app.i18n.t("settings_change"))),
        Line::from(Span::styled(
            format!("  {}", app.i18n.t("settings_theme_help")),
            Style::default().fg(Color::Rgb(theme.fg.0 / 2, theme.fg.1 / 2, theme.fg.2 / 2)).add_modifier(Modifier::ITALIC)
        )),
    ];
    ...
    let lang_text = vec![
        Line::from(format!("{}: {} {}", app.i18n.t("settings_language"), app.config.language, app.i18n.t("settings_change"))),
        Line::from(Span::styled(
            format!("  {}", app.i18n.t("settings_language_help")),
            Style::default().fg(Color::Rgb(theme.fg.0 / 2, theme.fg.1 / 2, theme.fg.2 / 2)).add_modifier(Modifier::ITALIC)
        )),
    ];
    let lang = Paragraph::new(lang_text)
        .style(lang_style);
    ...
    let lock_value = if app.config.auto_lock_minutes == 0 {
        format!("{}: {} {}", app.i18n.t("settings_autolock"), app.i18n.t("settings_disabled"), app.i18n.t("settings_adjust"))
    } else {
        format!("{}: {} {} {}", app.i18n.t("settings_autolock"), app.config.auto_lock_minutes, app.i18n.t("settings_minutes"), app.i18n.t("settings_adjust"))
    };
    let lock_text = vec![
    ...
    let lock = Paragraph::new(lock_text)
        .style(lock_style);
    ...
    let msg_text = vec![
        Line::from(format!("{}: {} {} {}", app.i18n.t("settings_timeout"), app.config.message_timeout_seconds, app.i18n.t("settings_seconds"), app.i18n.t("settings_adjust"))),
        Line::from(Span::styled(
    ...
    let help_text = vec![
        Line::from(""),
        Line::from(vec![
            Span::styled("[↑↓] ", Style::default().fg(Color::Rgb(theme.success.0, theme.success.1, theme.success.2))),
            Span::raw(format!("{}  ", app.i18n.t("cmd_navigate").replace("[↑↓] ", ""))),
            Span::styled("[←→] ", Style::default().fg(Color::Rgb(theme.success.0, theme.success.1, theme.success.2))),
            Span::raw(format!("{}  ", if app.i18n.t("settings_change").contains("change") { "Change" } else { "Cambiar" })),
            Span::styled("[s] ", Style::default().fg(Color::Rgb(theme.success.0, theme.success.1, theme.success.2))),
            Span::raw(format!("{}  ", app.i18n.t("cmd_save").replace("[s] ", ""))),
            Span::styled("[Esc] ", Style::default().fg(Color::Rgb(theme.error.0, theme.error.1, theme.error.2))),
            Span::raw(if app.i18n.t("cmd_cancel").contains("Cancel") { "Close" } else { "Cerrar" }),
        ]),
    ];
...
fn render_file_explorer(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let breadcrumb = Paragraph::new(format!("📂 {}", current_path))
        .style(Style::default().fg(Color::Rgb(
...
fn render_secrets_panel(f: &mut Frame, app: &mut App, area: Rect) {
        ...
        .block(
            Block::default()
                ...
                .title(if app.input_mode == InputMode::SearchingSecrets {
                    if app.use_regex {
                        format!("{} ({}, {})", app.i18n.t("search_secret_regex"), app.i18n.t("search_cancel"), app.i18n.t("search_regex"))
                    } else {
                        format!("{} ({}, {})", app.i18n.t("search_secret_normal"), app.i18n.t("search_cancel"), app.i18n.t("search_regex"))
                    }
                } else if app.use_regex {
                    format!("{} {}", app.i18n.t("press_search"), app.i18n.t("regex_active"))
                } else {
    ...
    let title = if let Some(path) = &app.file_path {
        ...
        let modified = if app.is_modified { app.i18n.t("modified") } else { "" };
        format!(
            "{}: {} ({}){}", 
            app.i18n.t("secrets"),
        ...
        .block(
            Block::default()
                ...
                .title(if app.input_mode == InputMode::SearchingKey {
                    format!("{} ({})", app.i18n.t("search_key_title"), app.i18n.t("search_cancel"))
                } else {
                    app.i18n.t("press_search").to_string()
                })
                .title_style(Style::default().fg(Color::Rgb(255, 167, 38)).add_modifier(Modifier::BOLD))
...
fn render_edit_modal(f: &mut Frame, app: &mut App) {
    ...
    let title = if app.input_mode == InputMode::Editing {
        format!("{} {} | {} | {}", app.i18n.t("edit_secret"), app.i18n.t("tab_switch"), app.i18n.t("enter_save"), app.i18n.t("esc_cancel"))
    } else {
        format!("{} {} | {} | {}", app.i18n.t("add_secret"), app.i18n.t("tab_switch"), app.i18n.t("enter_save"), app.i18n.t("esc_cancel"))
    };
    ...
    let key_info = if let Some(idx) = app.selected_key_index {
        if let Some(key) = app.age_keys.get(idx) {
            format!(
                " | {}: {}",
                app.i18n.t("key_info"),
                key.comment.as_ref().unwrap_or(&app.i18n.t("unnamed").to_string())
            )
        } else {
            format!(" | {}: {}", app.i18n.t("key_info"), app.i18n.t("key_info_none"))
        }
    } else {
        format!(" | {}: {}", app.i18n.t("key_info"), app.i18n.t("key_info_auto"))
    };
    ...
    if let Some(err) = &app.error_message {
        ...
        match app.input_mode {
            ...
            InputMode::Secrets => {
                ...
                let edit_cmds = if app.is_modified { format!(" | {}", app.i18n.t("cmd_save")) } else { String::new() };
            ...
            InputMode::SearchingSecrets => {
                if app.use_regex {
                    format!("{} [REGEX] - {}...", app.i18n.t("searching_secret"), app.i18n.t("search_regex"))
                } else {
                    format!("{} - {}...", app.i18n.t("searching_secret"), app.i18n.t("search_regex"))
                }
            }
            InputMode::ViewingValue => {
                format!("{} - {}", app.i18n.t("viewing_value"), app.i18n.t("scroll_json"))
            }
            InputMode::Editing => {
                format!("{} | {} | {} | {}", 
            ...
            InputMode::Confirming => format!("{} | {}", app.i18n.t("confirm_y"), app.i18n.t("cancel_n")),
            InputMode::Generating => app.i18n.t("generating").to_string(),
            InputMode::Help => format!("{} | [?] {}", app.i18n.t("close_help"), app.i18n.t("cmd_help")),
            InputMode::Settings => format!("{} | [←→] Change | {} | {}", app.i18n.t("cmd_navigate"), app.i18n.t("cmd_save"), app.i18n.t("cmd_cancel")),
            InputMode::ManagingKeys => {
                let selected_key = if app.key_manager_selected < app.age_keys.len() {
                    app.age_keys.get(app.key_manager_selected)
...
fn render_key_manager_modal(f: &mut Frame, app: &App) {
    ...
    let block = Block::default()
        ...
        .title_style(Style::default().fg(Color::Rgb(app.theme.primary.0, app.theme.primary.1, app.theme.primary.2)).add_modifier(Modifier::BOLD))
        .border_style(Style::default().fg(Color::Rgb(app.theme.success.0, app.theme.success.1, app.theme.success.2)));

    f.render_widget(block, area);

    let inner = area.inner(ratatui::layout::Margin::new(2, 2));
    
    let mut items: Vec<ListItem> = app.age_keys
        .iter()
        .enumerate()
        .map(|(i, key)| {
            ...
            let style = if i == app.key_manager_selected {
                Style::default().fg(Color::Rgb(app.theme.warning.0, app.theme.warning.1, app.theme.warning.2)).add_modifier(Modifier::BOLD)
            } else {
    ...
    let new_key_style = if app.key_manager_selected == app.age_keys.len() {
        Style::default().fg(Color::Rgb(app.theme.success.0, app.theme.success.1, app.theme.success.2)).add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(Color::Rgb(app.theme.success.0, app.theme.success.1, app.theme.success.2))
    };
    
    items.push(ListItem::new(format!("  ➕ New key: {}_", app.new_key_comment)).style(new_key_style));
...
fn render_confirm_key_deletion_modal(f: &mut Frame, app: &App) {
    ...
    let text = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("[y] ", Style::default().fg(Color::Rgb(239, 83, 80)).add_modifier(Modifier::BOLD)),
            Span::raw(format!("{}     ", if app.i18n.t("confirm_deletion_help").contains("Confirmar") { "Confirmar eliminación" } else { "Confirm deletion" })),
            Span::styled("[n] ", Style::default().fg(Color::Rgb(102, 187, 106)).add_modifier(Modifier::BOLD)),
            Span::raw(if app.i18n.t("cmd_cancel").contains("Cancelar") { "Cancelar" } else { "Cancel" }),
        ]),
    ];
...
fn render_creating_folder_modal(f: &mut Frame, app: &App) {
    ...
    let input = Paragraph::new(format!("{}\n\n{}_", app.i18n.t("enter_folder_name"), app.folder_name_buffer))
        .style(Style::default().fg(Color::Rgb(255, 255, 255)).bg(Color::Rgb(38, 50, 56)))
        .alignment(Alignment::Center)
        .block(
            Block::default()
...
fn render_renaming_file_modal(f: &mut Frame, app: &App) {
    ...
    let input = Paragraph::new(format!("{}\n\n{}_", app.i18n.t("enter_new_name"), app.rename_buffer))
        .style(Style::default().fg(Color::Rgb(255, 255, 255)).bg(Color::Rgb(38, 50, 56)))
        .alignment(Alignment::Center)
        .block(
            Block::default()
                ...
                .title_style(Style::default().fg(Color::Rgb(app.theme.primary.0, app.theme.primary.1, app.theme.primary.2)).add_modifier(Modifier::BOLD))
                .border_style(Style::default().fg(Color::Rgb(app.theme.warning.0, app.theme.warning.1, app.theme.warning.2)))
        );
...
fn render_confirm_file_deletion_modal(f: &mut Frame, app: &App) {
    ...
    let text = vec![
        ...
        Line::from(vec![
            Span::styled("[y] ", Style::default().fg(Color::Rgb(239, 83, 80)).add_modifier(Modifier::BOLD)),
            Span::raw(format!("{}     ", if app.i18n.t("confirm_deletion_help").contains("Confirmar") { "Confirmar" } else { "Confirm" })),
            Span::styled("[n] ", Style::default().fg(Color::Rgb(102, 187, 106)).add_modifier(Modifier::BOLD)),
            Span::raw(if app.i18n.t("cmd_cancel").contains("Cancelar") { "Cancelar" } else { "Cancel" }),
        ]),
    ];
...
fn render_selecting_sops_keys_modal(f: &mut Frame, app: &App) {
    ...
    let items: Vec<ListItem> = app.age_keys
        .iter()
        .enumerate()
        .map(|(i, key)| {
            let checked = app.selected_sops_keys.get(i).copied().unwrap_or(false);
            let radio = if checked { "(•) " } else { "( ) " };
            let name = key.comment.as_ref().map(|c| c.as_str()).unwrap_or("Unnamed");
...
fn render_editing_sops_config_modal(f: &mut Frame, app: &App) {
    ...
    let lines: Vec<Line> = app.edit_buffer
        .lines()
    ...
    let paragraph = Paragraph::new(lines)
        ...
        .style(Style::default().bg(Color::Rgb(38, 50, 56)).fg(Color::Rgb(189, 189, 189)));
...
fn render_selecting_sops_template_modal(f: &mut Frame, app: &App) {
    ...
    let templates = vec![
        (app.i18n.t("template_simple"), app.i18n.t("template_simple_desc")),
        (app.i18n.t("template_by_type"), app.i18n.t("template_by_type_desc")),
        (app.i18n.t("template_regex"), app.i18n.t("template_regex_desc")),
    ...
    let items: Vec<ListItem> = templates
        ...
        .map(|(i, (name, desc))| {
            ...
            let content = vec![
                Line::from(vec![
                    Span::styled(if i == app.selected_sops_template { "▶ " } else { "  " }, style),
